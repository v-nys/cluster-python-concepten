...
De veronderstelling is hier dat het hoofdproject, LBLP, aanwezig is op het systeem, met de meegeleverde plugins. Door de omgevingsvariable `LBLP_SOURCE_DIR` in te stellen op het pad naar die projectfolder en dan `envsubst -i pre-contents.lc.yaml > contents.lc.yaml` uit te voeren, kunnen plugins gebruikt worden zonder volledig hardgecodeerde paden in de broncode.
